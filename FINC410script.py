"""
XOP Options Hedging Analysis - FINC 410 Derivatives Project
Covers: Simple Options Hedge (Part 3) + Advanced Options Hedge (Part 4)

HOW TO USE:
1. Export CSVs from Bloomberg (see comments in each section)
2. Update the file paths and option parameters at the top of main()
3. Run: python xop_options_analysis.py
"""
...
def load_xop_prices(filepath):
    """
    Bloomberg export format expected:
      CSV with columns: Date, PX_LAST
      Date format: YYYY-MM-DD  (Bloomberg may export as MM/DD/YYYY — adjust if needed)

    Steps in Bloomberg to export:
      1. Type: XOP US Equity  <GO>
      2. Type: HP  <GO>  (Historical Prices)
      3. Set date range, daily frequency, select PX_LAST
      4. Actions → Export to Excel/CSV
    """
...
def simple_options_hedge(S0, K, T, r, sigma_iv, option_price_market,
    ...
    sigma_iv        : Implied vol (decimal) — from Bloomberg OMON or solved below
